          ...
          ...new Array(2).fill(0).map((_, index) => (
            <React.Fragment key={index}>
              {props.testimonials.map(({ text, image, name, role }, i) => (
                <div
                  className="p-8 rounded-3xl border border-border shadow-soft shadow-primary/10 max-w-xs w-full bg-card"
                  key={i}
                >
                  ...
                  <div className="flex items-center gap-3 mt-5">
                    ...
                    <div className="flex flex-col">
                      ...
                      <div className="leading-5 text-xs tracking-tight text-secondary">{role}</div>
                    </div>
                  </div>
                </div>
              ))}
